...
/* Flat mapping of old tj_node pointer → new tj_node pointer built
 * during the parallel DFS in build_tj_node_map().  Used to re-wire
 * parent_node / parent_doc after the structural copy and to remap
 * TokenSpan.owner in tok_spans_copy(). */
typedef struct { const tj_node *old_ptr; tj_node *new_ptr; } TjNodePair;
typedef struct { TjNodePair *pairs; int count; int cap; } TjNodeMap;

static void tn_map_push(TjNodeMap *m, const tj_node *old, tj_node *new_node) {
    if (m->count >= m->cap) {
        m->cap = m->cap ? m->cap * 2 : 16;
        m->pairs = realloc(m->pairs, (size_t)m->cap * sizeof(TjNodePair));
        if (!m->pairs) { fprintf(stderr, "taskjuggler-lsp: out of memory\n"); exit(1); }
    }
    m->pairs[m->count].old_ptr = old;
    m->pairs[m->count].new_ptr = new_node;
    m->count++;
}

static tj_node *tn_map_lookup(const TjNodeMap *m, const tj_node *old) {
    for (int i = 0; i < m->count; i++)
        if (m->pairs[i].old_ptr == old) return m->pairs[i].new_ptr;
    return NULL;
}

/* Recursively copy src's structure without wiring parent_node/parent_doc;
 * those are fixed up after the full copy via the map. */
static tj_node *tj_node_copy_r(const tj_node *src) {
    if (!src) return NULL;
    ...
    if (src->num_children > 0) {
        dst->children = calloc((size_t)src->num_children, sizeof(tj_node *));
        ...
        for (int i = 0; i < src->num_children; i++)
            dst->children[i] = tj_node_copy_r(src->children[i]);
    }
    return dst;
}

/* Walk old and new trees in parallel DFS to build the old→new map.
 * Both trees must be structurally identical (same child counts). */
static void build_tj_node_map(const tj_node *old_node, tj_node *new_node,
                               TjNodeMap *m) {
    if (!old_node || !new_node) return;
    tn_map_push(m, old_node, new_node);
    int n = old_node->num_children < new_node->num_children
            ? old_node->num_children : new_node->num_children;
    for (int i = 0; i < n; i++)
        build_tj_node_map(old_node->children[i], new_node->children[i], m);
}

tj_node *tj_node_copy(const tj_node *src) {
    if (!src) return NULL;
    tj_node *dst = tj_node_copy_r(src);
    TjNodeMap m = {0};
    build_tj_node_map(src, dst, &m);
    for (int i = 0; i < m.count; i++) {
        tj_node *d       = m.pairs[i].new_ptr;
        const tj_node *o = m.pairs[i].old_ptr;
        d->parent_node = o->parent_node ? tn_map_lookup(&m, o->parent_node) : NULL;
        d->parent_doc  = o->parent_doc  ? tn_map_lookup(&m, o->parent_doc)  : NULL;
    }
    free(m.pairs);
    return dst;
}

TokenSpan *tok_spans_copy(const TokenSpan *src, int n,
                           const tj_node *old_root, tj_node *new_root) {
    if (!src || n <= 0) return NULL;
    TjNodeMap m = {0};
    if (old_root && new_root)
        build_tj_node_map(old_root, new_root, &m);
    TokenSpan *dst = calloc((size_t)n, sizeof(TokenSpan));
    if (!dst) { fprintf(stderr, "taskjuggler-lsp: out of memory\n"); exit(1); }
    for (int i = 0; i < n; i++) {
        dst[i] = src[i];
        dst[i].text  = src[i].text ? strdup(src[i].text) : NULL;
        dst[i].owner = (src[i].owner && m.count)
                       ? tn_map_lookup(&m, src[i].owner)
                       : NULL;
    }
    free(m.pairs);
    return dst;
