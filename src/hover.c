...
/** One keyword's hover documentation. */
typedef struct {
    const char *keyword;  /**< TJP keyword, table sorted by this field */
...
const char *keyword_docs(const char *kw) {
    if (!kw) return NULL;
    const keyword_doc_entry *e =
        bsearch(kw, keyword_doc_table,
                sizeof(keyword_doc_table) / sizeof(keyword_doc_table[0]),
                sizeof(keyword_doc_table[0]), keyword_doc_cmp);
    return e ? e->docs : NULL;
}
