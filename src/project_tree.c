...
/* ── ProjectNode deep copy ──────────────────────────────────────────────── */

typedef struct { const ProjectNode *old_ptr; ProjectNode *new_ptr; } PnPair;
typedef struct { PnPair *pairs; int count; int cap; } PnMap;

static void pn_map_push(PnMap *m, const ProjectNode *old, ProjectNode *new_node) {
    if (m->count >= m->cap) {
        m->cap = m->cap ? m->cap * 2 : 16;
        m->pairs = realloc(m->pairs, (size_t)m->cap * sizeof(PnPair));
...
static ProjectNode *pn_map_lookup(const PnMap *m, const ProjectNode *old) {
    for (int i = 0; i < m->count; i++)
        if (m->pairs[i].old_ptr == old) return m->pairs[i].new_ptr;
    return NULL;
}

static ProjectNode *project_node_copy_r(const ProjectNode *src, PnMap *m) {
    if (!src) return NULL;
    ProjectNode *dst = calloc(1, sizeof(ProjectNode));
    if (!dst) { fprintf(stderr, "taskjuggler-lsp: out of memory\n"); exit(1); }
    dst->keyword          = src->keyword;
    ...
    pn_map_push(m, src, dst);
    if (src->num_children > 0) {
        dst->children = calloc((size_t)src->num_children, sizeof(ProjectNode *));
        if (!dst->children) { fprintf(stderr, "taskjuggler-lsp: out of memory\n"); exit(1); }
        dst->num_children     = src->num_children;
        dst->num_children_cap = src->num_children;
        for (int i = 0; i < src->num_children; i++)
            dst->children[i] = project_node_copy_r(src->children[i], m);
    }
    return dst;
}
...
ProjectNode *project_node_copy(const ProjectNode *src) {
    if (!src) return NULL;
    PnMap m = {0};
    ProjectNode *dst = project_node_copy_r(src, &m);
    for (int i = 0; i < m.count; i++) {
        ProjectNode       *d = m.pairs[i].new_ptr;
        const ProjectNode *o = m.pairs[i].old_ptr;
        d->parent_node = o->parent_node ? pn_map_lookup(&m, o->parent_node) : NULL;
    }
    free(m.pairs);
    return dst;
}
