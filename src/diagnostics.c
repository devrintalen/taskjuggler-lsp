 ...
 * tools/lsp_test.py compares LSP notifications positionally (responses
 * are matched by id, but notifications are an ordered list), so a worker
 * emitting publishDiagnostics concurrently with the coordinator would
 * interleave nondeterministically with the publishDiagnostics emitted
 * during inline notification dispatch and break golden-file diffs.
 * Today every call site sits inside a notification handler (or
 * revalidate_all_docs, which is called from one). */
